...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Fangde AI");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Fangde AI (Nightly)");
  });
      ...
      createStagePatchedDependencies(
        {
          ...
          "another-unused-package@1.0.0": "patches/another-unused-package@1.0.0.patch",
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.publish, [
        {
          provider: "generic",
          url: "https://ai-api.fdsure.com/downloads/desktop/latest",
        },
      ]);
  ...
  it.effect("uses FD metadata without retired identity entitlements", () =>
    Effect.gen(function* () {
