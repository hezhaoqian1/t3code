...
This fork is a local Desktop GUI for coding agents. An authenticated loopback Node server wraps provider CLIs and serves the Electron renderer.
...
### 3. Local by construction

The Electron app owns one authenticated server bound to `127.0.0.1`. New behavior must preserve that boundary: no LAN listeners, public origins, remote pairing, relay, or service-launcher fallback.

### 4. Desktop first

Desktop is the product surface. The web bundle is its renderer and local Vite development target; it is not a hosted or remotely shared application.
...
Be careful about accessing data, killing dev servers, and other actions that may damage the contributor's running local instance.
...
3. **Baking in origins.** Never set `VITE_HTTP_URL` or `VITE_WS_URL` for browser dev. Vite proxies `/api`, `/ws`, `/oauth`, and `/.well-known` to the loopback server.
...
- **Clients.** Desktop wraps the web renderer and adds the Electron shell/IPC. Shared local connection logic lives in `packages/client-runtime`.
...
- **Connection boundary.** The supported server is the authenticated loopback Desktop server.
...
- Dev servers and Vite bind to loopback. Do not add host overrides or expose them to another machine.
- Stop what you started, by the PID you tracked. See rule 1.

## Test data

Use generated fixtures and temporary worktree-local state for tests. Never access or start a server against `~/.t3/userdata`.
