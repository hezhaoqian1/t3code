...
export const resolveDesktopBackendPort = Effect.fn("resolveDesktopBackendPort")(function* (input: {
  ...
  if (input.isDevelopment && Option.isSome(input.configuredPort)) {
    return input.configuredPort.value;
  }
  const net = yield* NetService.NetService;
  return yield* net.reserveLoopbackPort("127.0.0.1");
});
...
const bootstrap = Effect.gen(function* () {
  ...
  yield* configuration.configurePort(backendPort);
  const backendHttpBaseUrl = new URL(`http://127.0.0.1:${String(backendPort)}`);
  yield* logBootstrapInfo("bootstrap reserved loopback backend endpoint", { port: backendPort });
  ...
  yield* electronProtocol.registerDesktopProtocol({
    ...
    backendOrigin: backendHttpBaseUrl,
  });
  yield* logBootstrapInfo("bootstrap resolved backend endpoint", {
    baseUrl: backendHttpBaseUrl.href,
  });
