...
it.layer(NodeServices.layer)("FeishuConnector", (it) => {
  it.effect(
    "uses bundled resources and clears stale errors for an authenticated account",
    () =>
      Effect.gen(function* () {
        ...
        const waitingMarker = path.join(root, "device-code-waiting");
        ...
        for (let attempts = 0; attempts < 100; attempts += 1) {
          if (yield* fileSystem.exists(waitingMarker)) break;
          yield* Effect.promise(() => NodeTimersPromises.setTimeout(10));
        }
        assert.isTrue(yield* fileSystem.exists(waitingMarker));

        const cancelled = yield* connector.disconnect.pipe(Effect.timeout("2 seconds"));
        assert.equal(cancelled.state.enabled, false);
        assert.equal(cancelled.state.authState, "not_authenticated");
        assert.equal((yield* Fiber.join(pendingConnect))._tag, "Failure");

        yield* fileSystem.remove(`${commandLog}.logout`);
        yield* fileSystem.writeFileString(readyMarker, "ready");
