// This file mostly exists because we want dev mode to use Fangde product metadata instead of Electron's.
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Fangde AI (Dev)" : "Fangde AI";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.fdsure.enterprise-ai.dev.${devBundleIdSuffix || "local"}`
  : "com.fdsure.enterprise-ai";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["fdai-dev"] : ["fdai"];
const LAUNCHER_VERSION = 15;
const defaultIconPath = NodePath.join(desktopDir, "resources", "icon.icns");
export const DEVELOPMENT_MAC_ICON_PNG_PATH = NodePath.join(
  repoRoot,
  "assets",
  "fd",
  "fangde-ai-1024.png",
...
function ensureDevelopmentIconIcns(runtimeDir) {
  ...
  if (!NodeFS.existsSync(DEVELOPMENT_MAC_ICON_PNG_PATH)) {
    return defaultIconPath;
  }

  const sourceMtimeMs = NodeFS.statSync(DEVELOPMENT_MAC_ICON_PNG_PATH).mtimeMs;
  if (
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Fangde AI name.
