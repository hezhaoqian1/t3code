...
async function loadYaml() {
  try {
    ...
    try {
      // Packaged Desktop supplies its production dependency directory. Using
      // createRequire here keeps the exporter independent of the employee's
      // PATH while still allowing the self-contained app bundle to provide
      // the YAML parser.
      const bundledModules = process.env.FD_PRESENTATION_NODE_MODULES;
      if (bundledModules) {
        const bundledRequire = createRequire(path.join(bundledModules, "package.json"));
        const yaml = bundledRequire("yaml");
        return (s) => yaml.parse(s);
      }
  ...
  await Promise.all(
    srcs.map(async (src, idx) => {
      ...
      if (/^(https?:\/\/|data:)/i.test(src)) {
        ...
        const local = path.join(projectRoot, src.replace(/^file:\/\/+/, "").replace(/\\/g, "/"));
...
function splitCustomPath(shape) {
  if (shape?.shapeName !== "custom" || typeof shape.path !== "string" || shape.viewBox) return;
  const t = shape.path.indexOf(";");
  if (t < 0) return;
  const [w, h] = shape.path.slice(0, t).split(",").map(Number);
  if (!(w > 0) || !(h > 0)) return;
  shape.viewBox = [w, h];
...
async function loadWasmExporter(wasmPath) {
  ...
  function getDataView() {
    if (
      ...
      (cachedDataView.buffer.detached === undefined && cachedDataView.buffer !== memory.buffer)
    ) {
  ...
  function exportPPTDToPPTXBytes(pptd, options, signature) {
    ...
    try {
      const ptr0 = isLikeNone(signature)
        ? 0
        : passStringToWasm(signature, exportsRef.__wbindgen_export, exportsRef.__wbindgen_export2);
      const len0 = S;
...
Env: KIMI_COOKIE, KIMI_ORIGIN`);
  ...
  const output =
    args.output || path.join(projectRoot, path.basename(manifestPath, ".pptd") + ".offline.pptx");
