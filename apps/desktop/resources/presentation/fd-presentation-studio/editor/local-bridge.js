...
async function writeIndexedFile(path, content) {
  if (state.desktopProjectPath && window.desktopBridge?.presentation?.writeFile) {
    await window.desktopBridge.presentation.writeFile({
      projectPath: state.desktopProjectPath,
      relativePath: path,
      content,
    });
...
window.__NEODECK_CONNECT__ = function neoDeckConnect(options) {
  ...
  if (state.exportMode) {
    ...
    const desktopPath = new URL(location.href).searchParams.get("desktopProjectPath");
    if (desktopPath && window.desktopBridge?.presentation?.readProject) {
      openDesktopProject(desktopPath).catch((error) => {
        console.error("[neodeck] desktop project failed", error);
        toast(`演示文稿载入失败：${error?.message || error}`, "error");
      });
      return {
...
async function openDesktopProject(projectPath) {
  setStatus("载入演示文稿…");
  const payload = await window.desktopBridge.presentation.readProject({ projectPath });
  state.desktopProjectPath = payload.projectPath;
  ...
  for (const file of payload.files || []) {
    if (file.dataUrl) {
      ...
      state.fileIndex.set(file.path, {
        kind: "file",
        getFile: async () => new File([blob], basename(file.path), { type: blob.type }),
      });
    } else {
      state.memoryFiles.set(file.path, file.content || "");
      state.fileIndex.set(file.path, {
        kind: "file",
        getFile: async () =>
          new File([file.content || ""], basename(file.path), { type: "text/plain" }),
      });
    }
  }
  const manifest = [...state.fileIndex.keys()].find((entry) =>
    entry.toLowerCase().endsWith(".pptd"),
  );
  if (!manifest) throw new Error("项目内没有 .pptd 文件");
  await loadDeckFromIndex(manifest, "方德任务 · 可写", { readOnly: false, editable: true });
}
