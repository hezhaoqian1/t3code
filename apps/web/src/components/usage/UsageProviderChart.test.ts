...
describe("buildDayColumns", () => {
  ...
  const byDay = new Map([
    [
      "2026-08-01",
      {
        ...
        costUsd: 30,
        totalTokens: 300,
        byProvider: new Map([["fd-deepseek" as const, { costUsd: 30, totalTokens: 300 }]]),
      },
    ],
    ...
    [
      "2026-08-03",
      {
        ...
        costUsd: 5,
        totalTokens: 50,
        byProvider: new Map([["fd-deepseek" as const, { costUsd: 5, totalTokens: 50 }]]),
      },
    ],
  ]);
  ...
  it("keeps the FD band value absolute rather than cumulative", () => {
    const [first] = buildDayColumns(days, byDay, "cost");

    expect(first?.bands).toEqual([{ provider: "fd-deepseek", value: 30 }]);
  });
