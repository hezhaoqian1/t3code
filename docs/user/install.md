...
普通任务默认使用公司管理的 `deepseek-flash`（DeepSeek V4.1 Flash）。需要更强推理能力时，可以在发送按钮左侧
选择 `V4 Pro`；切回 `V4 Flash` 不需要新建任务。只有平台授权的两个模型可以使用，账号凭据、
模型策略、AI 点数和 FD Skill 权限均由方德平台下发。
...
如果飞书授权页只允许选择个人账号，并提示当前应用没有企业组织使用权限，这不是桌面端缓存了
个人账号。请联系飞书管理员为当前 CLI 应用开通方德企业租户权限，然后回到“连接器”页面点击
“切换账号”重新授权。桌面端不会内置或下发飞书 App Secret。
